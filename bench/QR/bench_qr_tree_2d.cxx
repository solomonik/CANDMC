 ...
 **/
void qr_tree_2d_bench( int64_t const m,
                  int64_t const k, 
                  int64_t const b, 
                  int64_t const nprow, 
                  int64_t const npcol, 
  ...
  for (iter=0; iter<niter; iter++){
    ...
    QR_tree_2D(A,mb,m,k,b,myRank,numPes,0,0,cdt_row,cdt_col,cdt_glb);
  }
...
int main(int argc, char **argv) {
  ...
  if (transp_fact == 1)
    qr_tree_2d_bench(m, k, b, nprow, npcol, niter, myRank, numPes, 0, cdt_glb);
  else {
    ...
    qr_tree_2d_bench(m, k, b, nprow, npcol, niter, myRank, numPes, 0, cdt_glb_transp);
