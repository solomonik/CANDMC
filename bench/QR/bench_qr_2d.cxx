 ...
 **/
void qr_2d_bench( int64_t const m,
                  int64_t const k, 
                  int64_t const b2, 
                  int64_t const b, 
                  int64_t const nprow, 
  ...
  for (iter=0; iter<niter; iter++){
    ...
    pv.ccol = cdt_col;
    pv.cworld = cdt_glb;
    QR_2D_2D(A,mb,m,k,b2,b,&pv,NULL,0);
  }
  MPI_Barrier(cdt_glb.cm);
  time = MPI_Wtime()-time;

  if (myRank == 0)
    printf("2D QR with Householder reconstruction on a " PRId64 "-by-" PRId64 " matrix took %lf seconds/iteration, at %lf GFlops\n",
...
int main(int argc, char **argv) {
  ...
  if (transp_fact == 1)
    qr_2d_bench(m, k, b2, b, nprow, npcol, niter, myRank, numPes, 0, cdt_glb);
  else {
    ...
    qr_2d_bench(m, k, b2, b, nprow, npcol, niter, myRank, numPes, 0, cdt_glb_transp);
