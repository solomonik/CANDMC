...
void dcn_bench(int64_t const        n,
  ...
  if (myRank == 0){
    printf("BENCHMARKING 4D MM SUMMA / TOPO MPI_Bcast ALG\n");
  }
...
void d25_bench(int64_t const        n,
  ...
  if (myRank == 0){
    printf("BENCHMARKING 2.5D MM TOPO MPI_Bcast ALG\n");
  }
  ...
  MPI_Bcast(mat_A, b*b, MPI_DOUBLE, 0, cdt_kdir.cm);
  MPI_Bcast(mat_B, b*b, MPI_DOUBLE, 0, cdt_kdir.cm);
...
void ctb_bench(int64_t const        n,
  ...
  if (myRank == 0){
    printf("BENCHMARKING SUMMA TOPO MPI_Bcast ALG\n");
  }
...
int main(int argc, char **argv) {
  ...
  if (sqrt(numPes/c_rep)*sqrt(numPes/c_rep) != numPes/c_rep){
    if (myRank == 0)
      printf("This MM test needs square processor grid... exiting.\n");
    return 0;
  }
