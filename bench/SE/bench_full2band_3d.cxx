...
int main(int argc, char **argv) {
  int myRank, numPes, n, b, pr, pc, ipr, ipc, i, j, b_agg, iter, niter;
  double * loc_A;
  ...
  if(myRank == 0){
    ...
    printf("2.5D CANDMC full to band n = %d bw = %d b_agg = %d b = %d p = %d c = %d: sec/iteration: %f ", n, bw, b_agg, b, numPes, c_rep, time/niter);
    printf("Gigaflops (4/3)n^3: %f\n", ((4./3.)*n*n*n)/(time/niter)*1E-9);
  }
  free(loc_A);
