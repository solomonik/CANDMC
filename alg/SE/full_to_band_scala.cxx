...
void sym_full2band_scala(double  *    A,
  ...
  TAU_FSTART(sym_full2band_scala);
...
//  if (pv->cworld.rank == 0)
  ...
  pview qr_pv = *pv;
  QR_scala_2D(A+loc_row_offset, lda_A, n-b, b, b_sub, &qr_pv, Y, mb, desc_A, org_A, IA+b, JA);
  ...
  if (n-b > 0){
    sym_full2band_scala(A+loc_col_offset+loc_row_offset,lda_A,n-b,b,b_sub,pv,desc_A,org_A,IA+b,JA+b);
  }
  TAU_FSTOP(sym_full2band_scala);
}
